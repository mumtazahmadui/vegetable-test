...
export class ListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.service.getVegetable().subscribe((data) => {
        console.log("Inside list component data   " + data.length);
        this.searchVegetablesList = this.vegetables.filter((item) => item.toLowerCase().includes(data.toLowerCase()));
    }, () => {
      console.error("Inside empty data");
    });
